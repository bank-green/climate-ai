<template>
    <div v-clickaway="closePopup">
        <nav class="fixed z-50 top-0 p-4 contain inset-x-0">
            <div class="relative z-50 h-[60px] bg-primary-dark rounded-xl flex items-center justify-between pr-4">
                <NuxtLink to="/" class="py-4 px-6" @click="isOpen = false">
                    <!-- TODO: add icon -->
                    <span class=" text-white font-semibold text-xl">Climate-AI</span>
                </NuxtLink>
        ...
        <transition enter-active-class="duration-200 ease-out" enter-from-class="opacity-0 -translate-y-1"
            ...
            <div v-if="isOpen"
                ...
                <NuxtLink v-for="link in links" :key="link.href" :to="link.href" activeClass="bg-primary-light"
